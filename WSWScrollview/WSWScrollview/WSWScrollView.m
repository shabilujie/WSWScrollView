 ...
 *  识别是本地图片还是网络图片
 */
-(void)identifyLocalImagesGroupOrUrlImagesGrop
{
    _isLocalImagesArray = NO;
    NSArray *array;
   
    [self.dataSource imagesArrayForWSWScrollView:self];
//    if ([self.dataSource respondsToSelector:@selector(imagesArrayForWSWScrollView:)]) {
    
        array =[NSArray arrayWithArray:[self.dataSource imagesArrayForWSWScrollView:self]];
//    }
...
- (void)tapClick:(UITapGestureRecognizer *)tap{
    if ([self.delegate respondsToSelector:@selector(wswScroView:didSelectRowAtIndexPath:)]) {
        [self.delegate wswScroView:self didSelectRowAtIndexPath:_currentImageIndex + 1];
    }
}
...
-(void)setDataSource:(id<WSWScrollViewDataSource>)dataSource
{
    _dataSource = dataSource;
    [self identifyLocalImagesGroupOrUrlImagesGrop];
}
